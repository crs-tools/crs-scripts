...
if (!defined($ticket) || ref($ticket) eq 'boolean' || $ticket->{id} <= 0) {
	...
	my $olduuid1 = $props->{'Processing.Auphonic.ProductionID1'};
	my $olduuid2 = $props->{'Processing.Auphonic.ProductionID2'};
	# TODO get filename in a smarter way?
	...
	my $isIdentical = 0;
	my $newMD5 = getMD5($sourcefile) if defined($sourcefile);
	...
	# upload audio-file to auphonic and start the production, if source file changed
	if ($isIdentical == 0 || ! defined($olduuid1)) {
		...
		$props_new{'Processing.Auphonic.ProductionID1'} = $uuid1;
	} else {
		print "not repeating auphonic production for audio track1\n";
	}
	...
	if ($lang =~ /^..-../) {
		...
		if ($isIdentical == 0 || ! defined($olduuid2)) {
			...
			print "not repeating auphonic production for audio track2\n";
		}
	}
